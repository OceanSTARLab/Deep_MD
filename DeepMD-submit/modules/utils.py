...
def get_matrix(nrows, ncols, rank, noise_type, signal_type,
    ...
    """
        ...
        Inputs:
            nrows, ncols: Size of the matrix  矩阵的size
            rank: Rank of the matrix  矩阵的秩，假设这是已知的
...
def get_inp(tensize, const=10.0):
    ...
    """
    inp = torch.rand(tensize).to(device) / const
...
def get_2d_posencode_inp(H, W, n_inputs):
    ...
    inp = torch.autograd.Variable(torch.tensor(inp), requires_grad=True).to(device)
