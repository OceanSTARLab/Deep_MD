...
'''
    This is a modified version of the original code hosted at https://github.com/DmitryUlyanov/deep-image-prior
    
    Specifically
        - 1D version is added
        - The final sigmoid layer is turned off by default
'''
...
sys.path.append('E:/Deep tensor/DeepTensor-main/modules/deep_models')
# from deep_models import skip, texture_nets, resnet, unet
from .deep_models.skip import skip, skip1d
from .deep_models.texture_nets import get_texture_nets
from .deep_models.resnet import ResNet
from .deep_models.unet import UNet
...
def get_net(input_depth, NET_TYPE, pad, upsample_mode, n_channels=3,
    ...
    elif NET_TYPE == 'skip1d':
        net = skip1d(input_depth, n_channels,
                     num_channels_down=[skip_n33d] * num_scales if isinstance(skip_n33d, int) else skip_n33d,
                     num_channels_up=[skip_n33u] * num_scales if isinstance(skip_n33u, int) else skip_n33u,
                     num_channels_skip=[skip_n11] * num_scales if isinstance(skip_n11, int) else skip_n11,
                     upsample_mode=upsample_mode, downsample_mode=downsample_mode,
